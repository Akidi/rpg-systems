...
<script lang="ts">
	...
	const innerCircleColor = $derived.by(() => {
		switch (currency) {
			case 'copper':
				return 'rgba(255, 255, 255, 0.4)'; // Lighter for dark coin
			case 'silver':
				return 'rgba(128, 128, 128, 0.6)'; // Darker for light coin
			case 'gold':
				return 'rgba(184, 134, 11, 0.7)'; // Darker for light coin
			case 'platinum':
				return 'rgba(154, 154, 154, 0.7)'; // Darker for light coin
		}
	});
</script>
...
>
	<!-- Coin shadow/depth -->
	<circle cx="12" cy="13" r="10" fill={borderColor} opacity="0.3" />

	<!-- Main coin body -->
	<circle cx="12" cy="12" r="10" fill={color} stroke={borderColor} stroke-width="1" />

	<!-- Inner highlight circle -->
	<circle cx="12" cy="12" r="7" fill="none" stroke={innerCircleColor} stroke-width="1.5" />

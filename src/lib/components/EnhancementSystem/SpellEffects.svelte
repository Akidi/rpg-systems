...
<div class="card">
		...
		<div class="stat-card">
			...
			{#if spellEffects.enhancementCounts['Raw Damage']}
				<div class="stat-detail">
					+{formatPercentageChange(spellEffects.baseDamageAfterRaw, baseSpell.baseDamage)}% from raw
					damage ({spellEffects.baseDamageAfterRaw - baseSpell.baseDamage} damage)
				</div>
			{/if}
		</div>
	...
	<div class="damage-output">
		...
		<div class="output-main">
			<span class="total-damage">{spellEffects.damage * spellEffects.targets}</span> total damage ({spellEffects.damage}
			× {spellEffects.targets} target{spellEffects.targets > 1 ? 's' : ''})
		</div>
		<div class="output-detail">
			Expected crit damage: <span class="output-crit"
				>{spellEffects.critDamage * spellEffects.targets}</span
			>
		</div>

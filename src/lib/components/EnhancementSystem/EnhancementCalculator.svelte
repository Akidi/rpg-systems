<script lang="ts">
	...
	let costs = $derived.by(() => {
		...
		const enhancementAPCost =
			enhancements.length + (enhancements.find((e) => e.name === 'Multi-Target') ? 1 : 0);
		const enhancementMPCost =
			Math.ceil(baseSpell.mpCost * Math.pow(1.25, enhancements.length)) - baseSpell.mpCost;
	...
	let spellEffects = $derived.by(() => {
		...
		const lifeStealAmount =
			lifeStealPercent > 0 ? Math.ceil(totalDamageOutput * (lifeStealPercent / 100)) : 0;
		const manaStealAmount =
			manaStealPercent > 0 ? Math.ceil(totalDamageOutput * (manaStealPercent / 100)) : 0;
		const critLifeStealAmount =
			lifeStealPercent > 0 ? Math.ceil(critDamage * targets * (lifeStealPercent / 100)) : 0;
		const critManaStealAmount =
			manaStealPercent > 0 ? Math.ceil(critDamage * targets * (manaStealPercent / 100)) : 0;
	...
	let apRecovery = $derived.by(() => {
		...
		const apRecoveryAmount = Math.max(
			1,
			Math.floor(enhancementAPCost * (apRecoverySettings.recoveryPercent / 100))
		);
	...
	function addEnhancement(enhancement: Enhancement): void {
		if (
			enhancement.name === 'Multi-Target' &&
			enhancements.some((e) => e.name === 'Multi-Target')
		) {
			return;
		}
...
<div class="calculator-container">
	...
	<div class="desktop-layout">
		<div class="desktop-column">
			<CharacterStats {character} {costs} {apRecovery} {resetCharacter} {nextTurn} />

			<SpellConfiguration {baseSpell} bind:enemyCount />

			<EnhancementSettings {damageSettings} {stealSettings} {apRecoverySettings} />
		</div>

		<div class="desktop-column">
			<EnhancementSelector {availableEnhancements} {enhancements} {enemyCount} {addEnhancement} />

			<EnhancementStack {enhancements} {removeEnhancement} />
		</div>

		<div class="desktop-column">
			<SpellEffects {baseSpell} {spellEffects} />

...
import type { ITierConfig, Skill, EffectType, ProgressionStep, BinaryEffect, RankDelta } from '$lib/types/skill-planner';
...
export class SkillCalculator {
  ...
  calculateRankFromPoints(points: number): { rank: number; pointsInRank: number; pointsForRank: number } {
    let remainingPoints = points;
    let currentRank = 0;
    
    // Find the highest complete rank we can afford
    for (let rank = 1; rank <= 20; rank++) { // Assuming max 20 ranks
      const tier = this.getTierForRank(rank);
      if (!tier) break;
  ...
  private calculateCurveProgression(effect: EffectType, totalPointsSpent: number, maxPoints: number): number {
  ...
  private calculatePerRankWithProgression(effect: EffectType, currentRank: number, pointsInRank: number, pointsForRank: number): number {
  ...
  calculateEffectValue(effect: EffectType, totalPointsSpent: number, currentRank: number, pointsInRank: number, pointsForRank: number, maxPoints: number): number {

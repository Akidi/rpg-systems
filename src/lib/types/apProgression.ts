...
export const SCALING_FORMULAS: ScalingFormula[] = [
	{
		...
		type: 'universal',
		description: 'Natural progression to soft cap at level 50 (8 AP), with dex providing early acceleration',
...
export function calculatePostSoftCapBonus(character: Character, config: APConfig): number {
	if (character.level < config.softCapLevel && character.dexterity < config.softCapLevel * config.dexPerAP) {
...
export function calculateTotalAP(
	...
	const totalAP = Math.floor(baseAP + naturalLevelBonus + dexAcceleration + postSoftCapBonus + ascensionBonus);
	
	// Check if soft cap is reached
	const reachedSoftCap = formula.type === 'universal' ? 
	...
	return {
		...
		ascensionBonus,
		totalAP,
		formula: formula.type === 'universal' ? 
